...
test('journal authentication, writing, memories, and persistence', async t => {
  const dataDir = mkdtempSync(path.join(tmpdir(), 'diary-test-'));
  let server, base, cookie = '', entry, attachment;
  ...
  await start();
  const request = (url, method = 'GET', body, headers = {}) => fetch(base + url, { method, headers: { ...(cookie ? {Cookie:cookie} : {}), 'X-Journal-Request':'1', ...(body !== undefined && !Buffer.isBuffer(body) ? {'Content-Type':'application/json'} : {}), ...headers }, body: body === undefined ? undefined : Buffer.isBuffer(body) ? body : JSON.stringify(body) });
  ...
  await t.test('private records and export require sign-in', async () => {
    for (const url of ['/api/entries','/api/export','/api/attachments/00000000-0000-0000-0000-000000000000']) assert.equal((await request(url)).status,401);
  });
  await t.test('cross-origin mutations and short setup passwords are rejected', async () => {
    assert.equal((await request('/api/setup','POST',{password:'fixture-password-123'},{Origin:'https://untrusted.example'})).status,403);
    assert.equal((await fetch(base+'/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:'fixture-password-123'})})).status,403);
    assert.equal((await request('/api/setup','POST',{password:'short'})).status,400);
  });
  await t.test('first-run setup hashes a password and issues an HttpOnly session', async () => {
    const response = await request('/api/setup','POST',{password:'fixture-password-123'});
    assert.equal(response.status,200);const header=response.headers.get('set-cookie');assert.match(header,/HttpOnly/);assert.match(header,/SameSite=Strict/);cookie=header.split(';')[0];
    assert.equal((await request('/api/setup','POST',{password:'another-password-123'})).status,409);
    assert.deepEqual(await (await request('/api/status')).json(),{configured:true,authenticated:true});
  });
  ...
  await t.test('malformed JSON and oversized uploads fail cleanly', async () => {
    const malformed=await fetch(base+'/api/entries',{method:'POST',headers:{Cookie:cookie,'X-Journal-Request':'1','Content-Type':'application/json'},body:'{broken'});assert.equal(malformed.status,400);
  ...
  await t.test('restart keeps writing, attachments, and sign-in state', async () => {
  ...
  await t.test('logout revokes the session; correct password signs back in', async () => {
    assert.equal((await request('/api/logout','POST')).status,200);assert.equal((await request('/api/entries')).status,401);
    assert.equal((await request('/api/login','POST',{password:'wrong-password'})).status,401);
    const response=await request('/api/login','POST',{password:'fixture-password-123'});assert.equal(response.status,200);cookie=response.headers.get('set-cookie').split(';')[0];
  });
  ...
  await t.test('repeated failed sign-ins are rate limited', async () => {
    for(let n=0;n<10;n++)assert.equal((await request('/api/login','POST',{password:'wrong-password'})).status,401);
    assert.equal((await request('/api/login','POST',{password:'wrong-password'})).status,429);
  });
});
